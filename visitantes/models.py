...
class Visitante(models.Model):
		...
		visitar = models.CharField(
			verbose_name = "Quem deseja visitar:",
			max_length = 194,
			null=True,
			blank=True,
			)
		...
		def get_hora_da_saida(self):
			if sel.hora_da_saida:
				return hora_da_saida

			return "Horário de saída não registrado"

		def get_hora_altorizacao(self):
			if sel.hora_da_saida:
				return horario_altorizacao
		...
		def get_placa_do_carro(self):
			if sel.placa_do_carro:
				return placa_do_carro
